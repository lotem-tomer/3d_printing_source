...
void create_rotated_gm(const Gm_polyhedron& poly, Gm& rotated_gm) {
	...
	CGAL::convex_hull_3(points.begin(), points.end(), rotated_poly);
	std::transform(rotated_poly.facets_begin(), rotated_poly.facets_end(), rotated_poly.planes_begin(),
		Plane_equation());
	merge_coplanar_faces(rotated_poly);
